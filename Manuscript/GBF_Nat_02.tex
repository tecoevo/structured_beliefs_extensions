...
\begin{equation}
f_i(x)=a_{i,1}x_1+a_{i,2}x_2+\cdots+a_{i,n}x_n\nonumber\\
\end{equation}
...
In the previous subsection, we used an infinite population for the deterministic approach \citep{Hofbauer2003}. Now, we will proceed with a finite population.
...
Let's assume there are two strategies $A$ and $B$. Frequencies for $A$ and $B$ are $i$ and $(N-i)$.
...
\pi_B&=\frac {i}{N-1}b_1 + \frac{N-i-1}{N-1}b_0\label{eq:11}
\end{align}
Now, a question can arise, why are we using $i-1$ instead of $i$? We have to understand that instead of an infinitely large population, we are using a real finite population and for that, we have to remove the particular individual who is observing or analysing the situation, so we use $i-1$.\\
...
For the (1) scenario,
\begin{equation}
T_i^+=\frac{if_A}{if_A+(N-i)f_B}\frac{N-i}{N} \label{eq:12}
\end{equation}
For the (2) scenario,
\begin{equation}
T_i^-=\frac{(N-i)f_A}{if_A+(N-i)f_B}\frac{i}{N} \label{eq:13}
\end{equation}
For the (3) scenario,
\begin{equation}
1-T_i^+-T_i^- \label{eq:14}
\end{equation}
...
0 &= \underbrace{-T^-_j (\rho_j - \rho_{j-1})}_{y_j} + \underbrace{T^+_j (\rho_{j+1} - \rho_j)}_{y_{j+1}} \label{eq:15}
\end{align}
...
\vdots \notag \\
y_k &= \rho_k - \rho_{k-1} = \rho_1 \prod_{j=1}^{k-1} r_j \label{eq:16}\\
\vdots \notag \\
y_N &= \rho_N - \rho_{N-1} = \rho_1 \prod_{j=1}^{N-1} r_j \label{eq:17}
\end{align}
...
\sum_{k=1}^N y_k &= \rho_N-\rho_0=1 \label{eq:18}
\end{align}
Using \eqref{eq:16} and \eqref{eq:18} we can calculate the value of $\rho_1$.
\begin{equation}
1=\sum_{k=1}^N y_k=\sum_{k=1}^N \rho_1 \prod_{j=1}^{N-1}r_j=\rho_1(1+ \sum_{k=1}^{N-1}\prod_{j=1}^k r_j)\label{eq:19}
\end{equation}
So, we can write the fixation probability of one $A$ individual as,
\begin{equation}
\rho_1= \frac{1}{1+ \sum_{k=1}^{N-1}\prod{j=1}^k r_j} \label{eq:20}
\end{equation}
...
\begin{align}
\rho_i&= \sum_{j=1}^i y_k \nonumber\\
&= \rho_1 \sum_{k=1}^i \prod_{j=1}^{k-1}r_j\nonumber\\
&=\rho_1(1+ \sum_{k=1}^{i-1} \prod_{j=1}^k r_j)\nonumber\\
&= \frac{1+\sum_{k=1}^{i-1}\prod_{j=1}^k r_j}{1+ \sum_{k=1}^{N-1}\prod_{j=1}^k r_j}
\end{align}
...
&= \rho_A \prod_{j=1}^{N-1} r_j \label{eq:22}
\end{align}
