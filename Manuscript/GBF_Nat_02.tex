...
x^* &= \frac{3}{4} \nonumber
\end{align}
Over time, $3/4$ of the population will hunt hare and the $1/4$ of the population will hunt stag.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{SH.pdf}
    \caption{$x^*=\frac{3}{4}$, Internal Equilibrium for Stag Hunt Game}
    \label{fig:3}
\end{figure}
...
\cha{What will happen if this is a structured population?}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{SH_FP.pdf}
    \caption{Analytical plot for fixation probability of Stag Hunt Game}
    \label{fig:5}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{SH_FP_SIM.pdf}
    \caption{Simulation plot for fixation probability of Stag Hunt Game. Simulation number is $10^4$.}
    \label{fig:6}
\end{figure}

\subsection{Two Players with multiple strategies}
...
Let's assume that for the given strategies $i_R, i_S, i_P$ the ordering of average payoff is unique and $\pi_R$ is the highest value and $\pi_P$ is the lowest, i.e., $\pi_R > \pi_S > \pi_P$.\\
Then the transition probability $T_{P \to R}$ represents that in the next time step of the Moran process, an individual playing strategy $P$ is replaced by an individual playing strategy $R$. This signifies a birth-death event where one individual playing strategy $P$ is eliminated from the population and the strategy $R$ reproduces.
For $T_{P \to R}$ we can write that,
\begin{equation}
T_{P \to R} = \frac{i_R f_R}{i_R f_R + i_S f_S + i_P f_P}\times \frac{i_P f_P^{-1}}{i_R f_R^{-1} + i_S f_S^{-1} + i_P f_P^{-1}} \label{eq:40}
\end{equation}
For strong selection $w \to 1$ the condition will be $\lim_{w \to 1} T_{P \to R} =1$.\\
Now let's assume, the payoff orderings are not unique and two or more payoffs can be equal. Now we can write three conditions based on that.
\begin{enumerate}
\item If $\pi_R>\pi_S=\pi_P$, then the individual playing strategy $R$ is certainly selected for birth. But selection at
death will remove an single individual among two remaining
strategies $P$ and $S$ with probability determined by their abundance.
\begin{enumerate}
\item If $S$ is chosen for death we find,\\
$\lim_{w \to 1} T_{S \to R} =\frac{i_S}{i_S + i_P}$.
\item If $P$ is chosen for death we find,\\
$\lim_{w \to 1} T_{P \to R} =\frac{i_P}{i_S + i_P}$.
\end{enumerate}
\item If $\pi_R=\pi_S>pi_P$, then the individual playing strategy $P$ is selected for death. But selection at birth is random.
\begin{enumerate}
\item If $R$ is chosen for reproduction we find,\\
$\lim_{w \to 1} T_{P \to R} =\frac{i_R}{i_R + i_S}$.
\item If $S$ is chosen for birth we find,\\
$\lim_{w \to 1} T_{P \to S} =\frac{i_S}{i_R + i_S}$.
\end{enumerate}
\item If $\pi_R=\pi_S=\pi_P$, then the selection at birth and selection at death both are random.\\
$\lim_{w \to 1} T_{X \to Y} = \frac{i_X}{N} \times \frac{i_Y}{N}$\\
where, $X \in \{R, P, S\}$ and $Y \in \{R, P, S\}$.
\end{enumerate}
From the equations \eqref{eq:37}, \eqref{eq:38}, \eqref{eq:39} we can find the point sets in the simplex where two or more payoff is equal. $\pi_R=\pi_S=\pi_P$ is the only center point of the simplex.
We can write three equations based on this.
\begin{enumerate}
\item The first equation is $\pi_R=\pi_S$
\begin{align}
\pi_R &=\pi_S\nonumber\\
i_S-i_P &=i_P-i_R \nonumber\\
2i_P &= i_S+i_R \nonumber\\
2i_P &=i_S+N-i_P-i_S \nonumber\\
3i_P &= N \nonumber\\
i_P &= \frac{N}{3} \label{eq:41}
\end{align}
\item The second equation is $\pi_S=\pi_P$ and after solving it like \eqref{eq:41} we get $i_R=\frac{N}{3}$.
\item The third equation is $\pi_S=\pi_P$ and after solving it like \eqref{eq:41} we get $i_S=\frac{N}{3}$.
\end{enumerate}
Direction of the fixation is deterministic, this means individual with highest payoff is determined for reproduction and individual with lowest payoff is determined for death.
In cases, near the corners and the edges of the simplex the final state where the system fixates is determined by initial conditions.
But, in a larger area, the system fixates stochastically.
In this case where the system is symmetric, the fixation probability can be either one of these three $\rho_R, \rho_S, \rho_P$.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{RPS_FP.pdf}
    \caption{Simulation plot for fixation probability for Rock-Paper-Scissors Game. Simulation number is $10^3$.}
    \label{fig:7}
\end{figure}
