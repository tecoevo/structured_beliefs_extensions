...
The development of this system is a Markov chain process over the state space of all feasible $(i_R, i_P, i_S)$ triples fulfilling equation \eqref{eq:35}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RPS_FP.pdf}
    \caption{Fixation probability heatmap for the Rock-Paper-Scissors game in a finite population of size $N = 10$. Each ternary plot illustrates the probability that a specific strategy Rock (left), Paper (middle), or Scissors (right), fixates in the populations. The vertices of each triangle represent homogeneous populations, while interior points denote mixed populations. Color intensity corresponds to the probability of fixation for the strategy, with darker shades indicating higher probabilities. Results are based on $10^3$ stochastic simulations of the Moran process for each initial condition. These plots reveal how initial composition influences evolutionary outcomes in cyclic dominance dynamics.}
   \label{fig:6}
\end{figure}
