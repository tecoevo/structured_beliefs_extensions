...
\begin{figure}{h}
    \centering
      %\includegraphics[width=0.5\linewidth]{Prisoner's dilemma plot.pdf}
...
\begin{figure}{h}
    \centering
    %\includegraphics[width=0.5\linewidth]{chicken game plot.pdf}
    \caption{Chicken Game Dynamics}
    \label{fig: Replicator dynamics for Chicken Game}
\end{figure}
In this plot,less than the internal equilibrium point will cooperate, and higher than internal equilibrium will defect.\\
\subsubsection{Snowdrift Game}
Snowdrift game is a classical game in game theory in social dilemmas.where two players ($a$ and $b$) are heading toward each other on a snowy, one-lane road. Neither can pass unless one shovels the snow to clear the path. Both drivers have two choices: either shovel the snow (cooperate) or stay in their car and let the other person do all the work (defect).\\
We can write three conditions based on this,\\
One, if both stay in the car, they both will face a small loss.\\
Two, if one shovels and one stays,the one who stays will have a win and one who shovels will face lose.\\
Three, if they both stays, they'll have a huge loss.\\
we can write the payoff matrix based on actual values,which will follow the standard condition, T>R>S>P.\\
\[
\begin{array}{c|cc}
    & C & D \\
    \hline
...
\end{array}
\]
The internal or mixed equilibrium point will be,
\[xR+(1-x)S=xT+(1-x)P\]
\[3x+1-x=5x\]
\[x=\frac{1}{3}\]
\[x^*=\frac{1}{3}\]
If we notice, the chicken game and snowdrift game's mixed equilibrium point is same as their payoff structure, but where the difference lies?\\
The main difference between these two game is that in chicken game if the both players do not swerve and stay(D,D), they will have a head-on collision and it will be disastrous, so they will be wanting to avoid it aggressively,and they will go for a strategic bluffing or brinkmanship.It will increase the chances of defection. On the other hand,in snowdrift game both the player defects, no will face loss, but it would not appear as disastrous as they both can not move.Because of it, there will higher chances that they will take the risk of cooperation.
So in the case of snowdrift,$1/3$ of the population will defect over time and $2/3$ of the population will cooperate over time.\\
Let's have a plot using this idea,\\
\begin{figure}{h}
    \centering
    \includegraphics[width=0.5\linewidth]{Snowdrift game.pdf}
    \caption{Snowdrift Game Dynamics}
    \label{fig: Replicator dynamics for Snowdrift Game}
\end{figure}
In this case, less than the equilibrium point will defect and more than the equilibrium point will cooperate.
